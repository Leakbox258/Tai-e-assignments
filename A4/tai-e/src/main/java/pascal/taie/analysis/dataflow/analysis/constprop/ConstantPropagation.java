...
//        }
//        return fact;
        /// because of the entry is 'main', ignore its args
        return new CPFact();
    }
    ...
    public boolean transferNode(Stmt stmt, CPFact in, CPFact out) {
        ...
        stmt.getDef().ifPresent(lValue -> {
            if (lValue instanceof Var var) {
                out.remove(var);
            } else { // InstanceFieldAccess
                out.remove(((InstanceFieldAccess) lValue).getBase());
            }
        });
        ...
        for (RValue rvalue : stmt.getUses()) {
            stmt.getDef().ifPresent(lValue -> {
                if (lValue instanceof Var var) {
                    out.update(var, evaluate(rvalue, in));
                } else { // InstanceFieldAccess
                    ///  .getBase will get a non-int var
//                    out.update(((InstanceFieldAccess) lValue).getBase(), evaluate(rvalue, in));
                }
            });
        }
